...
タグの追加や削除、履歴の削除などの実装しています。

## 設計で悩んだポイント

初めてのReactで、「どこがエントリーポイントとなって動いているのか」を探るところからのスタートでした。

そんな状態だったので、コンポーネント・コンテキスト・それぞれのページにそれぞれどのぐらいの責任を持たせるべきかについて悩みながら開発を進めていきました。

例えば、`App.tsx`は**全体の共通レイアウトやルーティング定義を置く場所**として、それぞれのページは`pages/`ディレクトリ内に書きます。

これにより、`App.tsx`は「家の間取り」、`pages/`内は「各部屋内」のように責任の持たせる場所をわかりやすくできます。

また、今回はタイマーをアプリ全体で使えるようにしたいという要件がありました。  
それが`TimerContext`なのですが、`App.tsx`とどちらの方が上であるべきかと言う部分に悩みました。
...
しかし、どうやらブラウザの他のタブを開いている時、つまり**バックグラウンドタブでは`setInterval`の呼ばれる間隔が遅くなる**ようです。
...
3. バックグラウンドでの時間経過の計算。計算式は「`今の時間` - `タイマーを動かした時間`」。

![localStrageの画像](/assets/images/post/2024-07-21-solodo-log/screenshot.png)
...
原因は、`useEffect()`が最初に実行され、順序は決まっていないということです。

今回の場合、このアプリが最初に表示されたときに1番も2番も動き、`setSelectedTags()`が実行される前に`setItem`で空をsetしてしまっているということです。
...
`useState`と`useRef`で主にデータを管理し、そのためのロジックは`useEffect`の依存配列で起動することができるます。  
はじめは混乱しましたが、状態更新の非同期性・クロージャ問題にぶつかったとき、しっかりと時間を使って理解を進めました。

また、ブラウザの便利さも実感しました。  
このアプリを作るときに「**ネット環境がなくても動くものにしたい**」と思い、スタンドアロン型のアプリにしようかと考えていました。  
しかし、それだとインストールの壁が高くなり、気軽に使ってもらえないと感じ、ブラウザ上で実装したいという気持ちがありました。

そこで、いろいろと調べてみると、**PWA（Progressive Web App）**という技術があり、このアプリはキャッシュが消えない限りオフラインでも使えるようになっています。  
...
データの管理・共有をするためのものがコンテキスト、UIの表示・操作するためのものをコンポーネントという形で扱っています。

今回のシステムでは、タイマーの状態管理を`TimerContext`として定義し、それぞれのモーダル画面やボトムナビゲーションをコンポーネントとして定義しています。
